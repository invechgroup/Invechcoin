...
#if defined(NDEBUG)
# error "Invechcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x6e27329c4c9e7eb053ad46216533996bb6fdfed9fa056613b525cb5e29d85483");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Invechcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Invechcoin Signed Message:\n";
...
{
    // Invechcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 1000); // Invechcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Invechcoin: 3.5 days
static const int64 nTargetSpacing = 10 * 60; // Invechcoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Invechcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Invechcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x9432ed654c85cd3665908b0ec35fc7ba51e9fb75837dfe55101ec41f323e1504");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "invechcoin created on oct-2018";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 1000000 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("043ac2b8f0e7f82d2e0bfa239bd489325d52d81a310aa6cca71b22e19767e15cb81d6910a61009c776fcbb9746e8e55eb32074ebf752a506c3901ed4e2490e6a95") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1539683171;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084891880;

        if (fTestNet)
        {
            block.nTime    = 1539683158;
            block.nNonce   = 385693941;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x069c9ae802f7207d5b644d62c2da497a103f0f9b324cdbc59779338494e3938e"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfe, 0xc1, 0xb4, 0xdf }; // Invechcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("InvechcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("InvechcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static InvechcoinMiner(CWallet *pwallet)
{
    printf("InvechcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("invechcoin-miner");
    ...
    try { loop {
        ...
        printf("Running InvechcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("InvechcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&InvechcoinMiner, pwallet));
}
